...
def extract_from_jpeg_bytes(data: bytes) -> np.ndarray:
    """Decode a raw JPEG bytes object and return a (178,) float32 keypoint vector.

    Skips background segmentation (used in training pipeline) for demo speed.
    The model still performs well without it for live inference.
    """
    ...
    vec178, _ = extract_keypoints_from_frame(frame_rgb, get_models(), conf_thresh=0.35)
    return np.clip(vec178, 0.0, 1.0).astype(np.float32)


def extract_from_base64(b64_string: str) -> np.ndarray:
